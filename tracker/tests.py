...
class CustomSiteSearchTests(TestCase):
    ...
    def test_process_url_and_save_accepts_drcmart_product_url(self, mock_fetch, mock_extract):
        ...
        mock_extract.return_value = ExtractedProductData(product_name="Tecno Spark 40 8GB 256GB", price=299.99, currency="EUR", in_stock=True, sku_or_ean="TEC-40-256")
        listing, error = process_url_and_save("https://www.drcmart.com/fr/products/tecno-spark-40-8gb-256gb", expected_query="tecno spark 40", allowed_hosts=["drcmart.com"])
    ...
    def test_process_url_and_save_rejects_unstructured_site_pages_before_llm(self, mock_fetch, mock_extract):
        ...
        listing, error = process_url_and_save("https://www.example.com/product/abc", expected_query="smartphone", allowed_hosts=["example.com"])
        self.assertIsNone(listing)
    ...
    @unittest.skipUnless(os.getenv("AWS_ACCESS_KEY_ID") and os.getenv("AWS_SECRET_ACCESS_KEY"), "AWS Bedrock credentials not configured")
...
class SearchResultDisplayTests(TestCase):
    ...
    def test_fetch_and_clean_html_retries_with_backoff_on_transient_errors(self, mock_session_cls, mock_sleep):
        mock_session = mock_session_cls.return_value; mock_response = MagicMock(); mock_response.status_code = 200; mock_response.content = b"<html><body><script>bad()</script><h1>Produit test</h1></body></html>"; mock_session.get.side_effect = [requests.HTTPError("HTTP 429"), mock_response]
        result = fetch_and_clean_html("https://example.com/produit")
        self.assertIn("Produit test", result); self.assertEqual(mock_session.get.call_count, 2); mock_sleep.assert_called_once_with(1)

    @patch("tracker.services.requests.Session")
    def test_fetch_and_clean_html_replaces_invalid_bytes_in_remote_html(self, mock_session_cls):
        mock_session = mock_session_cls.return_value; mock_response = MagicMock(); mock_response.status_code = 200; mock_response.encoding = "utf-8"; mock_response.content = b"<html><body><h1>Produit \x80\xff test</h1><p>299,99 EUR</p></body></html>"; mock_session.get.return_value = mock_response
        result = fetch_and_clean_html("https://example.com/produit")
        self.assertIn("Produit", result); self.assertIn("299,99", result); self.assertIsNotNone(result)
    ...
    def test_process_url_and_save_falls_back_to_html_parsing_when_ollama_fails(self, mock_fetch, mock_ollama):
        mock_fetch.return_value = "<html><body><h1>Smartphone X</h1><div>299,99 €</div><p>En stock</p></body></html>"; mock_ollama.side_effect = Exception("ollama unavailable")
        listing, error = process_url_and_save("https://example.com/produit")
        self.assertIsNotNone(listing); self.assertIsNone(error); self.assertEqual(listing.product.name, "Smartphone X"); self.assertEqual(float(listing.price), 299.99)
    ...
    def test_process_url_and_save_rejects_unknown_or_zero_price_results(self, mock_fetch, mock_extract):
        mock_fetch.return_value = "<html><body>ok</body></html>"; mock_extract.return_value = ExtractedProductData(product_name="Unknown", price=0.0, currency="Unknown", in_stock=False, sku_or_ean=None)
        listing, error = process_url_and_save("https://mobile-rdc.com/produit")
        self.assertIsNone(listing); self.assertIn("invalides", error.lower())

    @patch("tracker.services._extract_llm_only")
    @patch("tracker.services.fetch_and_clean_html")
    def test_process_url_and_save_rejects_unrealistic_prices(self, mock_fetch, mock_extract):
        mock_fetch.return_value = "<html><body>ok</body></html>"; mock_extract.return_value = ExtractedProductData(product_name="Téléphone test", price=150000.0, currency="USD", in_stock=True, sku_or_ean="TEL-999")
    ...
    def test_search_and_scrape_product_deduplicates_and_skips_homepage_results(self, mock_ddgs, mock_process_url):
        mock_ddgs.return_value.__enter__.return_value.text.return_value = [{"href": "https://example.com/"}, {"href": "https://example.com/produit"}, {"href": "https://example.com/produit"}, {"href": "https://example.com/other"}]; mock_process_url.return_value = (SimpleNamespace(price=100), None)
        results, errors = search_and_scrape_product("iPhone 15", max_results=10)
        self.assertIsInstance(results, list); self.assertEqual(mock_process_url.call_count, 2); self.assertEqual(mock_process_url.call_args_list[0].args[0], "https://example.com/produit"); self.assertEqual(mock_process_url.call_args_list[1].args[0], "https://example.com/other"); self.assertEqual(errors, [])
    ...
    def test_search_and_scrape_product_does_not_fallback_when_selected_site_results_are_invalid(self, mock_ddgs, mock_process_url):
        mock_ddgs.return_value.__enter__.return_value.text.return_value = [{"href": "https://example.com/produit"}]; mock_process_url.return_value = (None, "Données extraites invalides ou page non exploitable.")
        results, errors = search_and_scrape_product("iPhone 15", site_filter="example.com", max_results=5)
        self.assertEqual(results, []); self.assertIn("Données extraites invalides", errors[0]); self.assertEqual(mock_process_url.call_count, 1); self.assertEqual(mock_process_url.call_args_list[0].args[0], "https://example.com/produit")

    @patch("tracker.services.process_url_and_save")
    @patch("tracker.services.DDGS")
    def test_search_and_scrape_product_supports_multiple_site_filters(self, mock_ddgs, mock_process_url):
        mock_ddgs.return_value.__enter__.return_value.text.side_effect = [[{"href": "https://www.example.com/produit"}], [{"href": "https://shop.net/produit"}]]; mock_process_url.side_effect = [(SimpleNamespace(price=150), None), (SimpleNamespace(price=120), None)]
        results, errors = search_and_scrape_product("iPhone 15", site_filter="example.com, shop.net", max_results=5)
        self.assertEqual(len(results), 2); self.assertEqual(errors, []); self.assertEqual(mock_process_url.call_count, 2); self.assertEqual(mock_ddgs.return_value.__enter__.return_value.text.call_count, 2)

    @patch("tracker.services.process_url_and_save")
    @patch("tracker.services.DDGS")
    def test_search_and_scrape_product_skips_comparator_pages(self, mock_ddgs, mock_process_url):
        mock_ddgs.return_value.__enter__.return_value.text.return_value = [{"href": "https://comparateur-prix.example/iphone-15"}, {"href": "https://shop.example/iphone-15"}]; mock_process_url.return_value = (SimpleNamespace(price=120.00, in_stock=True), None)
        results, errors = search_and_scrape_product("iPhone 15", max_results=5)
        self.assertEqual(len(results), 1); self.assertEqual(errors, []); self.assertEqual(mock_process_url.call_count, 1); self.assertEqual(mock_process_url.call_args_list[0].args[0], "https://shop.example/iphone-15")

    @patch("tracker.services.process_url_and_save")
    @patch("tracker.services.DDGS")
    def test_search_and_scrape_product_skips_low_quality_sources(self, mock_ddgs, mock_process_url):
        mock_ddgs.return_value.__enter__.return_value.text.return_value = [{"href": "https://forum.example/topic/iphone-15"}, {"href": "https://shop.example/iphone-15"}]; mock_process_url.return_value = (SimpleNamespace(price=120.00, in_stock=True), None)
        results, errors = search_and_scrape_product("iPhone 15", max_results=5)
        self.assertEqual(len(results), 1); self.assertEqual(errors, []); self.assertEqual(mock_process_url.call_count, 1); self.assertEqual(mock_process_url.call_args_list[0].args[0], "https://shop.example/iphone-15")
    ...
    def test_search_and_scrape_product_deduplicates_same_offer_across_sources(self, mock_ddgs, mock_process_url):
        mock_ddgs.return_value.__enter__.return_value.text.return_value = [{"href": "https://dup.example/produit"}, {"href": "https://dup.example/produit?ref=alt"}]
        mock_process_url.side_effect = [(SimpleNamespace(product_id=1, retailer_id=1, url="https://dup.example/produit", price=120.00, in_stock=True), None), (SimpleNamespace(product_id=1, retailer_id=1, url="https://dup.example/produit", price=115.00, in_stock=True), None)]
        results, errors = search_and_scrape_product("Produit doublé", max_results=5)
        self.assertEqual(len(results), 1); self.assertEqual(errors, []); self.assertEqual(float(results[0].price), 115.00)

    def test_duplicate_price_listing_is_rejected(self):
        retailer = Retailer.objects.create(name="Duplicate Shop", base_url="https://duplicate.example"); product = Product.objects.create(name="Produit dupliqué", sku_or_ean="DUP-001")
        PriceListing.objects.create(product=product, retailer=retailer, url="https://duplicate.example/produit", price="99.99", currency="EUR", in_stock=True)
        with self.assertRaises(ValidationError):
            PriceListing.objects.create(product=product, retailer=retailer, url="https://duplicate.example/produit", price="129.99", currency="EUR", in_stock=True)

    def test_cleanup_stale_listings_deactivates_old_entries(self):
        retailer = Retailer.objects.create(name="Old Shop", base_url="https://old.example"); product = Product.objects.create(name="Produit historique", sku_or_ean="OLD-001")
        PriceListing.objects.create(product=product, retailer=retailer, url="https://old.example/produit", price="150.00", currency="EUR", in_stock=True)
        listing = PriceListing.objects.get(product=product, retailer=retailer); PriceListing.objects.filter(pk=listing.pk).update(scraped_at=timezone.now() - timezone.timedelta(days=45)); deactivated = cleanup_stale_listings(days=30)
        self.assertEqual(deactivated, 1); listing.refresh_from_db(); self.assertFalse(listing.is_active); self.assertTrue(PriceListing.objects.filter(pk=listing.pk).exists())

    def test_process_url_and_save_rejects_homepage_urls(self):
        listing, error = process_url_and_save("https://example.com/"); self.assertIsNone(listing); self.assertIn("accueil", error.lower())

    @patch("tracker.services._extract_llm_only")
    @patch("tracker.services.fetch_and_clean_html")
    def test_process_url_and_save_rejects_unrelated_product_name(self, mock_fetch, mock_extract):
        mock_fetch.return_value = "<html><body>ok</body></html>"; mock_extract.return_value = ExtractedProductData(product_name="Casque Bluetooth", price=79.9, currency="EUR", in_stock=True, sku_or_ean="C-101")
        listing, error = process_url_and_save("https://www.example.com/produit", expected_query="iPhone 15")
        self.assertIsNone(listing); self.assertIn("non pertinent", error.lower())

    @patch("tracker.services._extract_llm_only")
    @patch("tracker.services.fetch_and_clean_html")
    def test_process_url_and_save_defaults_for_missing_extraction_values(self, mock_fetch, mock_extract):
        mock_fetch.return_value = "<html><body>ok</body></html>"; mock_extract.return_value = ExtractedProductData(product_name="", price=12.5, currency="", in_stock=False, sku_or_ean=None)
        listing, error = process_url_and_save("https://www.example.com/produit")
        self.assertIsNone(listing); self.assertIn("invalides", error.lower())
