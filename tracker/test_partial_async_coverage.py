...
class PartialAsyncCoverageTests(TestCase):
    def setUp(self):
        ...
        self.retailer = Retailer.objects.create(name="Marchand test", base_url="https://marchand.cd")
        self.listing = PriceListing.objects.create(
            ...
            price=Decimal("299.00"), currency="USD", in_stock=True,
        )
        self.run = SearchRun.objects.create(
            query="Infinix Hot 60 Pro 8GB 256GB", site_filter="all",
            target_merchants=3, status=SearchRun.STATUS_RUNNING,
        )

    def _success_job(self):
        return ScrapeJob.objects.create(
            search_run=self.run, query=self.run.query, url=self.listing.url,
    ...
    def test_global_partial_coverage_keeps_discovery_open(self):
        ...
        self.run.market_currency = ''
        self.run.save(update_fields=['market_code', 'market_currency'])
        self._success_job()
        self.assertEqual(_successful_run_merchants(self.run), 1)
    ...
    def test_partial_coverage_is_not_exhausted_while_pending_work_remains(self):
        ...
        ScrapeJob.objects.create(
            search_run=self.run, query=self.run.query,
    ...
    def test_partial_coverage_is_not_exhausted_without_any_success(self):
        ScrapeJob.objects.create(
            ...
            status=ScrapeJob.STATUS_FAILED, fetch_status="fetch_failed", finished_at=timezone.now(),
        )
