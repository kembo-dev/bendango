...
def fetch_and_clean_html(url):
    ...
    try:
        ...
        return None
    # Keep JSON-LD: it is the most reliable product source.
...
def _fallback_extract_html(html_snippet):
    ...
    # Only accept prices carrying an explicit currency; naked numbers are too risky.
...
def resolve_bedrock_model_id(model_name, region=None):
...
def extract_with_bedrock(html_snippet, model_name):
    if boto3 is None:
        raise RuntimeError("boto3 indisponible")
...
def extract_product_data(html_snippet, model_name=None):
    ...
    if structured:
        return ExtractedProductData(**structured)
    config = get_llm_config()
    selected = (model_name or config["default_model"]).strip()
    try:
        if config["provider"] == "bedrock":
            return extract_with_bedrock(html_snippet, selected)
        return extract_with_ollama(html_snippet, selected)
    except Exception:
        try:
            return extract_with_ollama(html_snippet, selected)
        except Exception:
...
def extract_with_llm(html_snippet, model_name=None):
    return extract_product_data(html_snippet, model_name)


def _is_relevant_product_match(product_name, query):
    return product_match_score(product_name, query).is_match
...
def _has_exploitable_product_structure(html_content):
    if not html_content:
        return False
    if extract_jsonld_product(html_content):
        return True
    ...
    has_product = bool(soup.find("h1")) and any(token in text for token in ("prix", "price", "stock", "panier", "cart", "sku", "product", "produit"))
...
def _find_existing_product(data, expected_query=None):
    if data.sku_or_ean:
        ...
        if found:
            return found
    candidates = Product.objects.filter(name__icontains=(data.brand or data.product_name).split()[0])[:50]
...
def process_url_and_save(url, model_name=None, expected_query=None, allowed_hosts=None):
    ...
    if not data.product_name.strip() or currency is None or not (0 < price <= Decimal("1000000000")):
        return None, "Données extraites invalides."
    ...
    if expected_query and host not in allowed and not match.is_match:
    ...
    with transaction.atomic():
        ...
        if not product:
            product = Product.objects.create(name=data.product_name.strip(), sku_or_ean=data.sku_or_ean, brand=data.brand, image_url=data.image_url)
        else:
            changed = []
        ...
        listing.extraction_source = data.source
...
def _is_homepage_url(url):
    ...
    return path in locale or (len(path.split("/")) == 1 and path not in {"products", "product", "produits", "produit"})


def _is_category_or_listing_url(url):
    path = (urlparse(url or "").path or "").lower()
    if ".oembed" in path: return True
    if any(t in path for t in ("/categorie/", "/category/", "/categories/", "/collections/", "/collection/", "/shop/", "/search/", "/ads/", "/annonces/", "/market/")): return True
    return False
...
def _collect_search_urls(search_term, max_results):
    ...
    with DDGS() as ddgs:
        for res in ddgs.text(search_term, max_results=max_results):
            ...
            if url and not _is_homepage_url(url) and not _is_category_or_listing_url(url) and not _is_comparator_url(url) and not _is_low_quality_source_url(url) and url not in urls:
                urls.append(url)
    return urls
...
def cleanup_stale_listings(days=30, product_id=None):
    ...
    qs = PriceListing.objects.filter(scraped_at__lt=cutoff, is_active=True)
    if product_id is not None: qs = qs.filter(product_id=product_id)
    # Preserve history: stale offers are deactivated, never deleted.
...
def search_and_scrape_product(product_query, site_filter="all", model_name=None, max_results=3):
    ...
    results, errors = [], []
    site_filter = (site_filter or "all").strip()
    ...
    terms = [f"site:{normalize_site_filter(site)[0]} {product_query}" for site in site_filters] if site_filters else [f"site:{d} {product_query} {country} prix" for d in local_domains] + [f"{product_query} {country} acheter prix", f"{product_query} acheter prix"]
    urls = []
    for term in terms:
        try:
            ...
            errors.append(f"Erreur recherche: {exc}")
        if urls and not site_filters: break
