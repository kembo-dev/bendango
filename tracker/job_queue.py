...
from tracker.markets import DEFAULT_MARKET_CODE, get_market
...
def _local_market_bonus(url: str, market_code: str = DEFAULT_MARKET_CODE) -> float:
    """Prefer candidates from the selected market without excluding global merchants."""
    market = get_market(market_code)
    domain = domain_from_url(url)
    normalized = (url or '').lower()
    bonus = 0.0
    if any(domain.endswith(tld) for tld in market.tlds):
        bonus += 18.0
    market_terms = tuple(market.hints) + tuple(market.cities)
    if any(term.lower() in domain for term in market_terms):
        bonus += 12.0
    elif any(term.lower() in normalized for term in market_terms):
        bonus += 6.0
    return min(24.0, bonus)


def _job_market_code(job: ScrapeJob) -> str:
    if job.search_run_id:
        return getattr(job.search_run, 'market_code', DEFAULT_MARKET_CODE) or DEFAULT_MARKET_CODE
...
def _queue_priority(job: ScrapeJob, successful_domains: set[str], health_cache: dict[str, float]) -> float:
    ...
    local_bonus = _local_market_bonus(job.url, _job_market_code(job))
...
def _claim_best_from_lane(status: str, now, window: int) -> ScrapeJob | None:
    candidates = list(ScrapeJob.objects.filter(status=status, available_at__lte=now).select_related('search_run').order_by('available_at', 'created_at')[:window])
    if not candidates:
        return None
    successful_domains = {}
    health_cache = {}
